    ...
    for step_idx in range(steps):
        ...
        if ctrl_kwargs["pd_height_control"] or xy_height_hold:
            ...
            if ctrl_kwargs["pd_height_control"] and not xy_height_hold:
                # Match training: PD tracks the target altitude. Fixed hold_z is
                # reserved for the explicit --xy-height-hold demo mode.
                height_target = env.target_position[:, 2]
            else:
                height_target = torch.full_like(robot_z, hold_z)
            hold_vz = (
                float(ctrl_kwargs["hold_kp"]) * (hold_z - robot_z)
...
def main():
    ...
    ctrl_kwargs = {
        ...
        "pd_height_control": bool(ckpt_args.get("pd_height_control", False)),
        ...
        "hold_z": args.hold_z,
        "hold_kp": float(args.hold_kp if args.hold_kp is not None else ckpt_args.get("height_hold_kp", 0.8)),
        "hold_kd": float(args.hold_kd if args.hold_kd is not None else ckpt_args.get("height_hold_kd", 1.2)),
